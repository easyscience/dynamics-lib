...
from .components import Lorentzian, ModelComponent
...
class BrownianTranslationalDiffusion(DiffusionModel):
    ...
    def __init__(
        ...
        """
        super().__init__(name=name)
        self.diffusion_coefficient = Parameter(
            name="diffusion_coefficient", value=diffusion_coefficient, fixed=False
        )
    ...
    def create_components(
        self, Q: Union[Number, list, np.ndarray], component_name="Lorentzian"
        ...
        for i in range(len(Q)):
            ...
            width = Parameter.from_dependency(
                name="Gamma",
            ...
            lorentzian_component = Lorentzian(
                name=f"{component_name}", width=width, area=self.scale
            )
...
class JumpDiffusion(DiffusionModel):
    """Lorentzian model with half width half maximum equal to :math:`Dq^2`"""
    ...
    def __init__(
        ...
        self.scale = Parameter(name="scale", value=scale, fixed=False)
        self.tau = Parameter(name="tau", value=tau, fixed=False)
        self._width = None
        self._EISF = None
        self._QISF = None
    ...
    def calculate_width(self, Q: np.ndarray) -> np.ndarray:
        ...
        D = self.diffusion_coefficient.value  # TODO: handle units properly
        tau = self.tau.value
        self._width = (0.06582 * D * Q**2) / (1 + 0.1 * (D * Q**2 * tau))
        return self._width
    ...
    def write_width_dependency_expression(self, Q) -> str:
        ...
        # return f"(D * {Q}**2) / (1 + (D * {Q}**2 * tau))"
        return f"(0.06582*D * {Q}**2) / (1 + 0.1*(D * {Q}**2 * tau))"

    def write_width_dependency_map_expression(self) -> Dict[str, str]:
        """
        Write the dependency map expression for the width as a function of q to make dependent Parameters.
        """
        return {"D": self.diffusion_coefficient, "tau": self.tau}
    ...
    def get_parameters(self) -> List[Parameter]:
        ...
        List[Parameter]
        """
        params = [self.diffusion_coefficient, self.tau, self.scale]
        return params
