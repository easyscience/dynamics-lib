...
class Job(JobBase):
    ...
    def plot_data_and_model(self,intensity_min=0.0, intensity_max=0.06,
        ...
        INTENSITY_MIN_VANADIUM=intensity_min
        INTENSITY_MAX_VANADIUM=intensity_max

        ENERGY_MIN_VANADIUM = energy_min * sc.Unit('meV')
        ENERGY_MAX_VANADIUM = energy_max * sc.Unit('meV')
    ...
    def use_fit_as_resolution(self,job):
        """
        Use the fit from the job as the resolution model.
        Args:
        ...
        print(len(job._analysis))
        print(len(self._analysis))
        for i in range(len(self._analysis)):
