...
class TestExperiment:
    ...
    def test_extract_x_y_weights(self):
        # WHEN
        ...
        # THEN
        x, y, weights = experiment._extract_x_y_weights(Q_index=Q_index)

        # EXPECT
        assert np.array_equal(x, experiment.energy.values)
        assert np.array_equal(y, experiment.data.values[Q_index])
        assert np.array_equal(
            weights,
            1 / experiment.data.variances[Q_index] ** 0.5,
        )
